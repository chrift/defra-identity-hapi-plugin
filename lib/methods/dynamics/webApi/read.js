...
module.exports = (
   ...
   * @return {Promise<Object>}
   */
  const readEnrolment = async (contactIds, serviceRoleIds = undefined, accountIds = undefined, serviceUserLinkIds = undefined, serviceIds = undefined, includeAllServiceRecords = false, top = 50) => {
    const request = await readEnrolment.buildRequest(contactIds, serviceRoleIds, accountIds, serviceUserLinkIds, serviceIds, includeAllServiceRecords, top)

    return got(request).then(decodeResponse)
  }

  readEnrolment.buildRequest = async (contactIds, serviceRoleIds = undefined, accountIds = undefined, serviceUserLinkIds = undefined, serviceIds = undefined, includeAllServiceRecords = false, top) => {
