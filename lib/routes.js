...
module.exports = (
  ...
  server.route({
    ...
    handler: async (request, h) => {
      try {
        debug(`Received user at ${config.redirectUri}`)

        const { payload, method, query } = request

        debug({ payload, method, query })
        ...
        if (method === 'post' && config.passRequestToCacheMethods) {
          debug({ passRequestToCacheMethods: config.passRequestToCacheMethods, redirectUrl: `${config.redirectUri}?${qs.stringify(payload)}` })
